function SSLManager(config) {
    ...
    me.backupScripts = function backupScripts() {
        ...
        return me.exec([
            ...
            [ me.cmd, "cd %(letsencryptPath); hash tar 2>/dev/null && echo tar || yum install tar -f; tar -czvf backup.tar . >> %(logPath); mv backup.tar %(backupPath)", {
                logPath: logPath,
                backupPath: backupPath,
    ...
    me.restoreScripts = function restoreScripts() {
        ...
        return me.execAll([
            ...
            [ me.cmd, "hash tar 2>/dev/null && echo tar || yum install tar -f; mkdir -p %(settingsPath) && cd %(settingsPath) && tar -xzvf %(backupPath)/backup.tar > %(logPath)", {
                backupPath: backupPath,
                logPath: logPath,
